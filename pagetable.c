...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
        ...
        // Get evicted pte
        pgtbl_entry_t *evicted_pte = coremap[frame].pte;
        
		// Check pte dirty bit to see if need to write to swap
		if (evicted_pte->frame & PG_DIRTY) {
			// Update PTE to indicate that virtual page is no longer in physical memory
			// make the page to be evicted invalid
			
			evicted_pte->swap_off = swap_pageout(frame >> PAGE_SHIFT, evicted_pte->swap_off);
			evicted_pte->frame |= PG_ONSWAP;
			evict_dirty_count++;
		}
		// If the page is not dirty, simply discard it
		else {
			evicted_pte->frame = evicted_pte->frame & ~PG_ONSWAP;
			evict_clean_count++;
		}

		// Reset valid and reference bits for evicted page table entry
		evicted_pte->frame &= ~PG_VALID;
		evicted_pte->frame &= ~PG_REF;
...
char *find_physpage(addr_t vaddr, char type) {
	...
	pgdir_entry_t *pg_table = &pgdir[idx];

	
	// Use vaddr to get index into 2nd-level page table and initialize 'p'

	unsigned idx_second_level = PGTBL_INDEX(vaddr);
	p = &((pgtbl_entry_t*)(pg_table->pde))[idx_second_level];


	// Check if p is valid or not, on swap or not, and handle appropriately
	if (!(p->frame & PG_VALID)) {

        p -> frame = ((allocate_frame(p) << PAGE_SHIFT) | PG_VALID);

        // If the entry is invalid and not on swap
		if (!(p->frame & PG_ONSWAP)){

            //(simulated) physical frame should be allocated and initialized (using init_frame)
        ...
        // If the entry is invalid and on swap
		else {
			//(simulated) physical frame should be allocated and filled by reading the page data from swap
            swap_pagein(p->frame >> PAGE_SHIFT, p->swap_off);
            
            // check for type L and I
            if (type == 'S' || type == 'L') {
